...
class Paginacion {
	...
	public function __construct($paginaActual = 1, $registrosPP = 10, $totalRegistros = 0) {
		$this->paginaActual = (int) $paginaActual;
		$this->registrosPP = (int) $registrosPP;
		$this->totalRegistros = (int) $totalRegistros;
	}

	public function offset() {
		return $this->registrosPP * ($this->paginaActual-1);
	}

	public function totalPaginas() {
		return ceil($this->totalRegistros / $this->registrosPP);
	}
	...
	public function enlaceAnterior() {
		...
		if($this->paginaAnterior()) {
			$html .= "<a class='paginacion__enlace paginacion__enlace--texto' href='?page={$this->paginaAnterior()}'>« Anterior</a>";
		}
		return $html;
	}
	...
	public function enlaceSiguiente() {
		...
		if($this->paginaSiguiente()) {
			$html .= "<a class='paginacion__enlace paginacion__enlace--texto' href='?page={$this->paginaSiguiente()}'>Siguiente »</a>";
		}
		return $html;
	}
